...
const { fromRawTemplate } = require('./compile-body')
...
module.exports = (context, body) => {
  ...
  if (body.template && body.text) {
    ...
    if (body.template && body.template.template && body.template.templateName) {
      const validationError = 'template and templateName can not be specified together'
      context.log(errorMsg, validationError)
    ...
    } else if (body.template && body.template.template) {
      let template
      try {
        template = fromRawTemplate(body.template.template)
      } catch {
        template = undefined
      }
